...
do
	if [ -f $audio_file_path ]; then
	 	...
	 	timestamp=$(echo $audio_file_name | sed -r -n 's#^([0-9]+)-max-([0-9.]+)-mid-([0-9.]+)\.wav$#\1#p')
	 	max_amplitude=$(echo $audio_file_name | sed -r -n 's#^([0-9]+)-max-([0-9.]+)-mid-([0-9.]+)\.wav$#\2#p')
	 	mid_amplitude=$(echo $audio_file_name | sed -r -n 's#^([0-9]+)-max-([0-9.]+)-mid-([0-9.]+)\.wav$#\3#p')
			...
			if [ "$concat_file_queue_count" -gt 0 ]; then
				...
				if [ "$concat_file_queue_count" -gt 50 ]; then
					...
					# todo move to function
		 			...
		 			final_filepath="/usr/sleeptalk/records-to-render/${final_filename}"
